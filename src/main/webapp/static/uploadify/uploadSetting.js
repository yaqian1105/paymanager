var ProjectName = window.location.pathname.substring(0, window.location.pathname.indexOf("/",2))
/* this is a 例子
var uploadParams = {
		"limitWidth":640,
...
function upload(inputFileId,inputName,preViewId,fileType,queueID,fileSizeLimit,callBack,uploadParams) {
	...
	var action = ProjectName+"/user/upload/pic";
	if(uploadParams) {
	...
	$("#"+inputFileId).uploadify({
		'swf': ProjectName+'/static/uploadify/uploadify.swf',
		'uploader': action,
		'fileObjName': 'files', //必须，和以下input的name属性一致                   
		'auto'  : true, //是否自动开始  
